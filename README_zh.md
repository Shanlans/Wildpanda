...
兼容任何能读取 Markdown 规范的 LLM 编程 Agent。
...
- **函数级注释模板** — 标准 `@<owner>-comment` 格式（owner 来自 `project_profile.yaml`）：输入/输出接口、关键步骤、关键参数、状态/依赖。
...
└───────────────────────┬───────────────────────┘
                        ▼
              ┌───────────────────┐
              │  Read AGENTS.md   │ ← 强制读取顺序
              └─────────┬─────────┘
                        ▼
          ┌───────────────────────────┐
          │  Continuity Check (连续性) │
          │  • chat_index.md          │ ← 发现任务
          │  • stale detection        │ ← 心跳 > 4h?
          │  • governance sync        │ ← 上游更新?
          └─────────────┬─────────────┘
                        ▼
          ┌───────────────────────────┐
          │  Governance Decision      │
          │  • continue task?         │ ← 继续已有任务
          │  • new task?              │ ← 创建新任务
          │  • non-task request?      │ ← 非任务请求
          └─────────────┬─────────────┘
                        ▼
        ┌─────────────────────────────────┐
        ...
        └───────────────┬─────────────────┘
          ...
          ┌───────────────────────────┐
          │  Rule Reconfirmation      │
          │  Re-read AGENTS.md        │ ← 防止漂移
          └─────────────┬─────────────┘
                        ▼
          ┌───────────────────────────┐
          │  Session End (会话结束)    │
          │  Heartbeat stops          │ ← 下次会话检测超时
          └───────────────────────────┘
</pre>
...
| **readme-update** | 同步 README 到当前框架状态 | 自动（发布/贡献时）/ 手动："更新 readme" |

## 💬 指令参考

在对话中说出以下任意短语即可触发对应的治理技能：

| 操作 | 指令 |
|---|---|
| **初始化治理** | "初始化治理"、"引导治理"、"新项目接入"、"bootstrap governance" |
| **同步上游更新** | "检查治理更新"、"同步治理"、"sync governance" |
| **贡献变更到上游** | "agent 更新"、"agent 变更发布"、"贡献回上游"、"contribute back" |
| **准备发布** | "发布"、"正式发布"、"发布新版本"、"prepare release" |
| **查询调用关系** | "查调用关系"、"谁调用了"、"调用链"、"call graph" |
| **发现流程** | "分析 flow"、"梳理调用链"、"发现 flow"、"discover flow" |
| **更新 README** | "更新 readme"、"同步 readme"、"刷新 readme"、"update readme" |
| **严格验证** | 消息中包含 `???` 强制执行完整 8 步治理管线 |
...
| | |
|---|---|
| **主要目标** | LLM 编程 Agent（Claude Code、Cursor 等） |
